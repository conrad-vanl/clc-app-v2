...
export class dataSource extends ContentfulDataSource {
  getStartTime = async ({ fields, sys }) => {
    ...
    // a little contrived...
    try {
      const breakout = await this.context.dataSources.Breakouts.getFromEvent(
        sys.id
      );
...
export const resolver = {
  Event: {
    ...
    registered: async (
      { sys, fields },
      { nodeId },
      { dataSources: { UserLike, Cache } },
      { parentType }
    ) => {
      const key = `/${createGlobalId(sys.id, parentType.name)}/${
        parentType.name
      }/count`;
      let registered = await Cache.get({ key });
      if (registered === undefined || registered === null) {
        registered = await UserLike.model.count({
          where: { nodeId: String(sys.id), nodeType: parentType.name },
        });
  ...
  Query: {
    myScheduleFeed: (root, args, { dataSources: { FeatureFeed } }) =>
      FeatureFeed.getFeed({
        type: 'apollosConfig',
        args: {
          section: 'SCHEDULE_FEATURES',
          time: Math.round(Date.now() / 1000 / 60),
        }, // makes the ID change every minute
      }),
  ...
  Registration: {
    node: async ({ nodeId }, args, { dataSources: { Event } }) =>
      Event.getFromId(nodeId),
  },
  Mutation: {
    register: async (
      root,
      args,
      { sessionId, dataSources: { Event, UserLike, Person, Cache } }
    ) => {
      ...
      const registered = await UserLike.model.count({
        where: { nodeId: String(event?.sys?.id), nodeType: globalId.__type },
      });
      ...
      if (!capacity || registered < capacity) {
        ...
        await Cache.set({
          key: `/${sessionId}/${args.nodeId}/Event`,
          data: true,
          expiresIn: 3600,
        });
        await Cache.set({
          key: `/${args.nodeId}/Event/count`,
          data: undefined,
          expiresIn: 3600,
        });
      }
